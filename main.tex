...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{media/fig_model_comparison.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{media/fig_qbnet_probmap.png}
    \caption{QB-Net probability map (left), thresholded binary detection at 0.5 (center), and ground truth label (right) for Scene~4. The probability map shows that fire-positive regions cluster near P(fire) $= 1.0$, while boundary regions exhibit intermediate confidence that a tuned threshold could resolve more cleanly.}
    \label{fig:qbnet-probmap}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{media/fig_sampler_map.png}
    \caption{Scene~4 prediction map under the fire-aware sampler (right) compared to the input image (left) and ground truth (center). The sampler causes the model to predict fire across nearly the entire scene, visually illustrating the precision collapse reported in Table~\ref{tab:sampler-fail}.}
    \label{fig:sampler-map}
\end{figure}
