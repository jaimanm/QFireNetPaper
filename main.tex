...
\begin{itemize}[leftmargin=2em,topsep=2pt,itemsep=2pt]
    ...
    \item We identify and quantify a severe train/test domain shift induced by Sen2Fire's scene-based split (a $5.3\times$ difference in aerosol), and show it is the binding performance ceiling: a uniform random re-split lifts the strongest classical model by $+8.75$ Fire $F_1$, roughly four times the quantum margin.
    \item We show that a Feature Pyramid Network decoder with the aerosol channel dropped (mode~4), a purely classical change, matches the best quantum result ($31.13$), reinforcing that the bottleneck inductive bias is not the limiting factor under the official split.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.95\textwidth]{media/v2_fig/v2_fig_main_results.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.95\textwidth]{media/v2_fig/v2_fig_loss_ablation.png}
