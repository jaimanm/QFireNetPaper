...
\paragraph{Hybrid quantum-classical neural networks.} Recent studies replace selected classical layers of a deep network --- typically convolutions or dense bottlenecks --- with parameterized quantum circuits, motivated by the exponentially large Hilbert space these circuits operate in. Two recent designs targeting the U-Net bottleneck inform our work directly. Qu-Net's QuFeX module~\cite{qufex} splits the qubit register and applies $U_1$ ($R_X{+}R_Z$) and $U_2$ ($R_X{+}R_Y$) blocks with a CZ entangling ring; QB-Net~\cite{qbnet} encodes inputs with $R_Y$ embedding, applies a $U_3$ rotation on every qubit, and entangles via an open CNOT ladder. To both bottlenecks we add data re-uploading~\cite{data-reuploading}, re-encoding the input before every variational layer to lift effective expressivity without increasing the trainable parameter count. We introduce this modification because the original QuFeX design encodes the input only once. We adopt both circuits as drop-in bottleneck replacements inside an otherwise identical full-width U-Net, which isolates any classical/quantum performance difference to the circuit itself rather than to backbone changes. A known limitation in this family of architectures is gradient vanishing on deep or wide circuits (the barren-plateau phenomenon~\cite{barren-plateaus}), constraining practical circuit depth on near-term hardware and partly motivating the modest qubit counts ($n \le 8$) and layer depths ($L \le 2$) we evaluate.
...
\begin{table}[ht]
    ...
    \begin{tabular}{lccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Parameter footprint of the original Sen2Fire baseline U-Net and our width-reduced compact variant. Reducing the encoder/decoder width from $64{\to}1024$ to $48{\to}768$ removes $\approx 7.55$\,M parameters. A preliminary balanced local sanity check (Table~\ref{tab:prelim-accuracy}) indicates the width reduction does not catastrophically degrade accuracy; a matched-epoch full train/validation/test run (Table~\ref{tab:classical-runs}) confirms the gap is small (Fire $F_1$ $28.34$ vs.\ $28.71$ for the base-64 matched baseline).}
    \label{tab:param-counts}
\end{table}

\paragraph{Preliminary accuracy check.} To verify that the width reduction does not catastrophically degrade segmentation quality, we ran a small balanced local sanity check comparing the two widths on CPU. Both models were evaluated on the same balanced local subsets: a 48-patch training subset (24 fire / 24 non-fire) and a 24-patch test subset (12 fire / 12 non-fire). Critically, the width-64 baseline used the existing fully trained checkpoint, whereas the width-48 compact model was trained locally for only a single epoch, so the comparison is biased \emph{against} the compact model. Under these conditions the width-64 baseline reached $90.95\%$ pixel accuracy and the width-48 compact model reached $87.04\%$ (Table~\ref{tab:prelim-accuracy}); for reference, the existing full-pipeline baseline reaches $93.41\%$ on the same metric. The $\approx 3.9$\,pp gap is consistent with the compact model's single-epoch training budget rather than a fundamental loss of capacity, but we report it transparently rather than overstating accuracy preservation. We present this only as a preliminary check motivating the compact backbone.

\paragraph{Full matched-conditions run.} We subsequently trained the width-48 compact model to completion under the same protocol used for the matched-conditions baseline and every other classical/quantum experiment in this paper (mode 5, 5 epochs, learning rate $1\times10^{-4}$, weight decay $5\times10^{-4}$, fire-class weight 10, seed 1234). On the full Sen2Fire test split it reaches Fire $F_1 = 28.34$ (P $=39.29$, R $=22.16$, Fire IoU $=16.51$, mIoU $=54.97$; Table~\ref{tab:classical-runs}), only $0.37$ Fire $F_1$ below the matched-conditions base-64 baseline ($28.71$) and within noise of it on mIoU ($54.97$ vs.\ $54.99$). This confirms that the $43.73\%$ parameter reduction preserves segmentation accuracy at matched training conditions, resolving the open question left by the preliminary local check above.
...
To establish whether the quantum gain over the classical baseline is matched by a sufficiently expressive \emph{classical} architectural change, we also evaluate a Feature Pyramid Network~\cite{fpn} variant of the U-Net. The FPN-U-Net retains the same full-width (base-64) encoder used by the quantum variants but replaces the U-Net's symmetric upsampling decoder with a top-down pyramid. At every encoder stage, a $1\times1$ lateral convolution unifies channel widths to a shared FPN width (256), the top-down feature is bilinearly upsampled and added to the lateral, and a $3\times3$ smoothing convolution removes aliasing. Pyramid levels $P_1$--$P_5$ are then resampled to the highest resolution and summed before a $1\times1$ output convolution. A 2D dropout layer ($p=0.5$) at the bottleneck regularizes the deepest features, and training combines weighted cross-entropy with MixUp~\cite{mixup} and CutMix~\cite{cutmix} sample mixing. 
...
\begin{table}[ht]
    ...
    \begin{tabular}{llcccccc}
        ...
        Quantum FPN (SE-gate)$^{c}$               & FPN-64   & 4 & random & 5   &  0.00 &  0.00 & 47.87 \\
        \addlinespace
        ...
        \bottomrule
    \end{tabular}
    \caption{Complete experiment ledger. Best test Fire $F_1$ run per experiment (epoch count shown). Only the scene-based Sen2Fire group is directly comparable to the published benchmark; random-split, validation, and cross-dataset rows are reported separately. $^{a}$The matched-conditions 5-epoch run ($F_1=28.71$) is the anchor used for all classical/quantum comparisons in the body; the classical U-Net peaks at $29.83$ given a longer (20-epoch) budget. $^{b}$Negative result. $^{c}$The only recovered quantum-FPN run collapses to $F_1=0$; a provisional $34.43$ was previously reported for this slot (Table~\ref{tab:random-split}, footnote $\ddagger$).}
    \label{tab:ledger}
\end{table}
