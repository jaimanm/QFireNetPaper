...
\begin{itemize}[leftmargin=2em,topsep=2pt,itemsep=2pt]
    \item We implement and compare two variational quantum bottleneck ansatzes --- QuFeX and QB-Net --- as drop-in replacements for the deepest U-Net convolution, augmenting both with data re-uploading. Under matched conditions on Sen2Fire, both modestly exceed a classical baseline (QB-Net Fire $F_1 = 31.18$, QuFeX $30.79$, vs.\ $28.71$).
    \item We introduce a compact (base-48) U-Net that reduces parameters by $43.73\%$ with negligible loss in test-set Fire $F_1$.
    \item We identify and quantify a severe train/test domain shift induced by Sen2Fire's scene-based split (a $5.3\times$ aerosol-channel shift), and show it is the binding performance ceiling: a uniform random re-split lifts the strongest classical model by $+8.75$ Fire $F_1$, roughly four times the quantum margin.
    \item We show that a purely classical change --- a Feature Pyramid Network decoder with the aerosol channel dropped (mode~4) --- matches the best quantum result ($31.13$), reinforcing that the bottleneck inductive bias is not the limiting factor under the official split.
    \item We demonstrate cross-dataset portability by transferring the pipeline to the California Burned Areas (CaBuAr) dataset.
    \item We report transparent negative results --- an amplitude-encoding bottleneck that produces NaN gradients, and a fire-aware oversampler that collapses precision.
\end{itemize}
...
Two things stand out. First, the FPN architecture benefits more from the split change than the vanilla U-Net does ($+8.75$ vs.\ $+3.98$ Fire $F_1$), suggesting that the FPN's multi-scale fusion is particularly hurt by the train/test distribution mismatch in the scene-based split --- consistent with the high val/test gap we observed during FPN training under mode 5 (Section~\ref{sec:fpn-extended}). Second, under the same random-split protocol, the quantum FPN variant falls $5.3$ points \emph{below} the classical FPN (Table~\ref{tab:random-split}). We caution against over-reading this comparison: the quantum FPN uses a small channel-attention quantum gate at the bottleneck rather than the per-spatial-location QuFeX/QB-Net bottleneck inserts that generated the $\approx +2$ quantum margin under the scene-based split (Sections~\ref{sec:qufex}--\ref{sec:qbnet}). The two are different quantum architectural designs and this result does not directly determine whether QuFeX or QB-Net retain their margin once the split-induced ceiling is removed. \textbf{We do not have QuFeX or QB-Net runs under the random-split protocol yet}, and acquiring them is the single most informative remaining experiment in this work (Section~\ref{sec:futurework}).
...
Given the $2.5\%$ training fire-pixel rate, we tested a sampler-level rebalancing strategy: a \texttt{WeightedRandomSampler} configured so that fire-containing patches were drawn at a target rate of $35\%$, roughly $3.5\times$ their natural prevalence. The model and loss function were otherwise unchanged from the classical baseline. The result was a sharp redistribution of the operating point, not a net improvement (Table~\ref{tab:sampler-fail}): precision collapsed and recall surged, but Fire $F_1$ roughly halved and overall accuracy fell by almost thirty points. The interaction is straightforward in hindsight: the weighted cross-entropy already up-weights the fire class by a factor of ten, and compounding that with sampler-level oversampling pushed the model into over-confident fire predictions across most of the image. Sampler rebalancing alone is not a free improvement over weighted CE on this dataset; combining the two requires careful re-tuning of \texttt{fire\_class\_weight}, which we leave to future work.
