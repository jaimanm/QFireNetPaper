...
A direct consequence of this analysis is that dropping the aerosol channel at training time (\texttt{mode = 4}, SWIR-only) should narrow the train/test gap. We confirm this empirically in Section~\ref{sec:fpn-extended} below on a classical FPN-augmented variant.

\subsection{Classical Architectural Extensions: FPN and Mode-4}
\label{sec:fpn-extended}

The domain-shift analysis in Section~\ref{sec:domainshift} predicts that dropping the aerosol channel should narrow the train/test generalization gap, even though aerosol is the single most-separating band on the training split (see Appendix~\ref{sec:bandaucs}). We test this on the classical FPN variant from Section~\ref{sec:fpn-method}.
