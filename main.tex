...
\begin{table}[H]
    ...
    \begin{tabular}{lcccccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Random-split sensitivity diagnostic. Re-partitioning the data at random removes the separation between train and test. The classical U-Net gains $+3.98$ Fire $F_1$, and the classical FPN gains $+8.75$ from this change alone. \emph{These numbers are not directly comparable to the published Sen2Fire benchmark}, which is defined on the scene-based split; we report them as a controlled diagnostic to isolate the contribution of the split itself.}
    \label{tab:random-split}
\end{table}

One thing stands out: the FPN architecture benefits more from the split change than the vanilla U-Net does ($+8.75$ vs.\ $+3.98$ Fire $F_1$), suggesting that the FPN's multi-scale fusion is particularly hurt by the train/test distribution mismatch in the scene-based split --- consistent with the high val/test gap we observed during FPN training under mode 5 above. \textbf{We do not have QuFeX or QB-Net runs under the random-split protocol yet}, and acquiring them is the single most informative remaining experiment in this work (Section~\ref{sec:futurework}).
