...
\paragraph{Train/test domain shift is the binding ceiling.} We documented the $\approx 5\times$ aerosol-channel shift between training and test scenes in Section~\ref{sec:domainshift}, along with the $\approx 2\times$ fire-pixel-rate shift. An intervention we tried was to replace the decoder with an FPN and drop the aerosol channel.Two architecturally orthogonal interventions --- replacing the bottleneck with a quantum circuit, or replacing the decoder with an FPN and dropping the aerosol channel --- converge on the same $\approx 31\%$ Fire $F_1$ ceiling on the held-out test scene. We also found that removing the scene-based split entirely lifts classical FPN Fire significantly, shown by an $8.75$-point gain. We read all three lines of evidence as showing the binding constraint on Sen2Fire performance under the official scene-based protocol is the domain shift itself, not the inductive bias of the bottleneck. A natural follow-up is to evaluate The natural follow-up is to evaluate either method (or both) on a dataset whose splits are constructed to be more iid, which is part of our motivation for the CaBuAr cross-dataset work in Section~\ref{sec:cabuar}.
