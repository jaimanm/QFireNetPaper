...
\begin{table}[H]
    ...
    \begin{tabular}{lcccccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Random-split sensitivity diagnostic. Re-partitioning the data at random into removes the separation between train and test. The classical U-Net gains $+3.98$ Fire $F_1$, and the classical FPN gains $+8.75$ from this change alone. $^{\dagger}$The quantum FPN uses a small quantum channel-attention gate at the bottleneck (squeeze-excite style), architecturally distinct from the QuFeX/QB-Net inserts described in Sections~\ref{sec:qufex} and~\ref{sec:qbnet}. $^{\ddagger}$This value could not be reproduced from recovered training artifacts: the only recovered quantum-FPN run for this configuration collapses to a degenerate all-background prediction (Fire $F_1 = 0$), so the $34.43$ figure should be treated as provisional pending re-verification.}
