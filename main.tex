...
\begin{itemize}[leftmargin=2em,topsep=2pt,itemsep=2pt]
    ...
    \item We identify and quantify a severe train/test domain shift induced by Sen2Fire's scene-based split (a $5.3\times$ aerosol-channel shift), and show it sets a substantial performance ceiling under the official split: a uniform random re-split lifts the strongest classical model by $+8.75$ Fire $F_1$.
...
\begin{figure}[H]
    ...
    \caption{Scene-based vs.\ random $70/15/15$ split Fire $F_1$ for each architecture (Table~\ref{tab:random-split}). The classical FPN gains $+8.75$ pp from mixing alone. The Quantum FPN has no scene-based result (see table footnote).}
